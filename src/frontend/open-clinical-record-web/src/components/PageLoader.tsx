...
export function PageLoader({
  ...
  if (variant === 'skeleton') {
    return (
      <div className={`ocr-loader ocr-loader-skeleton${className ? ` ${className}` : ''}`} role="status" aria-live="polite" aria-busy="true">
  ...
  return (
    <div
      className={`ocr-loader ocr-loader-pulse${variant === 'session' ? ' session' : ''}${className ? ` ${className}` : ''}`}
      role="status"
      aria-live="polite"
      aria-busy="true"
    >
