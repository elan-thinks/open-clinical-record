              ...
              {calMode === 'day' ? (
                ...
                <div className="week-wrap">
                  <div className="week-header">
                    ...
                    {days.map((d, i) => {
                      ...
                      return (
                        <div
                          key={iso}
                          className={`wh${iso === todayIso ? ' today' : ''}`}
                          onClick={() => setDate(iso)}
                          role="presentation"
                        >
                  ...
                  <div className="week-body">
                    ...
                    {days.map((d) => {
                      const iso = toIsoDate(d);
                      const colAppts = filteredWeek.filter((a) => a.appointmentDate === iso);
                      return (
                        <div key={iso} className={`week-col${iso === todayIso ? ' today-col' : ''}`}>
                          {HOURS.map((h) => (
                            <div key={h} className="hour-line" />
                          ))}
                          {iso === todayIso && nowLineTop != null && (
                            <div className="now-line" style={{ top: nowLineTop }} />
                          )}
                          {colAppts.map((a) => renderBlock(a, true))}
                        </div>
                      );
                    })}
                  </div>
