...
export function PatientChartPage() {
  ...
  async function onSaveVitals(e: FormEvent) {
    ...
    setSaving(true);
    try {
      const draftVisit = chart?.visits.find((v) => (v.status ?? '').toLowerCase() === 'draft');
      const payload = {
  ...
  async function onSaveConsultation(e: FormEvent) {
    ...
    setError(null);
    try {
      const draftVisit = chart?.visits.find(
        (v) => (v.status ?? '').toLowerCase() === 'draft',
      );
      const openAppt = appointments.find((a) => a.id === draftVisit?.appointmentId)
        ?? appointments.find((a) =>
      ...
      const clinicalPayload = {
        visitType: draftVisit?.visitType ?? openAppt?.appointmentType ?? 'Consultation',
        chiefComplaint: consultationComplaint.trim() || undefined,
        bloodPressure: consultationBp.trim() || undefined,
        pulse: numOrUndef(consultationPulse),
        temperatureC: numOrUndef(consultationTemp),
        spo2: numOrUndef(consultationSpo2),
      ...
      if (draftVisit) {
        try {
          await documentVisit(patientId, draftVisit.id, clinicalPayload);
        } catch (docErr) {
          console.warn('documentVisit failed; creating a new visit instead', docErr);
    ...
    <div className="chart-page">
            ...
            <div className="panel">
              ...
              <div className="info-grid">
                <div className="info-item">
                  ...
                  <div className="val">{chart.firstName} {chart.lastName}</div>
                </div>
                <div className="info-item">
                ...
                <div className="info-item">
                  ...
                  <div className="val">{formatDob(chart.dateOfBirth)} ({age})</div>
                </div>
                <div className="info-item">
                  <label>Phone</label>
              ...
              {chart.allergies.length === 0 ? (
                ...
                <div className="tag-list">
                  {chart.allergies.map((a) => (
                    <span key={a.id} className="tag allergy">{allergyLabel(a)}</span>
                  ))}
                </div>
              )}
            </div>
                ...
                <div className="list-rows">
                  {chart.visits.slice(0, 5).map((v) => (
                    <div key={v.id} className="list-row">
                      <div>
                        <div>{fmtDate(v.visitDate)} — {v.visitType}</div>
                        <div className="muted">{v.clinicianName ?? 'Clinician'} · {v.status}</div>
                      </div>
                      <span className={`status-pill ${v.status === 'Completed' || v.status === 'Final' ? 'done' : 'open'}`}>{v.status}</span>
                    </div>
                  ))}
                </div>
              )}
        ...
        <>
            ...
            {chart.medicalHistory.length === 0 ? (
              ...
              chart.medicalHistory.map((h) => (
                <div key={h.id} className="list-row">
                  <div>
                    <div>{h.description}</div>
                    <div className="muted">{h.category} · {h.isActive ? 'Active' : 'Resolved'}</div>
                  </div>
                </div>
              ))
            )}
          ...
          <div className="panel">
            ...
            <form onSubmit={onAddHistory}>
              ...
              <button type="submit" className="btn-primary" disabled={saving || isDeceased}>Save history</button>
            </form>
            <form onSubmit={onAddAllergy} style={{ marginTop: 16 }}>
              ...
              <button type="submit" className="btn-primary" disabled={saving || isDeceased}>Save allergy</button>
            </form>
          </div>
        </>
      )}
      ...
      {tab === 'vitals' && (
        <>
          ...
          <div className="panel">
            <div className="panel-head"><div><div className="panel-title">Recorded vital signs</div><div className="muted">Vitals are attached to the corresponding clinical encounter.</div></div></div>
        ...
        <div className="panel">
          ...
          <form onSubmit={onSaveConsultation}>
            ...
            <div className="form-actions">
              <button type="button" className="btn-ghost" onClick={() => selectTab('visits')}>Cancel</button>
              <button type="submit" className="btn-primary" disabled={saving || isDeceased}>{saving ? 'Saving...' : 'Save consultation'}</button>
            </div>
          </form>
        </div>
      )}
      ...
      {tab === 'appointments' && (
        <div className="panel">
          <div className="panel-head">
            ...
            <button type="button" className="panel-link" onClick={() => navigate(`/appointments/new?patientId=${chart.patientId}`)} disabled={isDeceased}>+ Book</button>
          </div>
            ...
            <div className="list-rows">
              {appointments.map((a) => (
                <div key={a.id} className="list-row">
                  <div>
                    <div>{a.appointmentDate} · {a.startTime} — {a.appointmentType}</div>
                    <div className="muted">{a.providerName ?? 'Provider'} · {a.reason || '—'}</div>
                  </div>
      ...
      <ConfirmDialog
        ...
        description={
          <>
            ...
            <label className="death-form-label" htmlFor="death-date">Date of death (optional)</label>
            <input id="death-date" type="date" className="input death-form-input" value={deathDate} onChange={(e) => setDeathDate(e.target.value)} max={new Date().toISOString().slice(0, 10)} />
            <label className="death-form-label" htmlFor="death-note">Notes (optional)</label>
            <textarea id="death-note" className="input death-form-input" rows={3} placeholder="Cause context, place, or administrative note…" value={deathNote} onChange={(e) => setDeathNote(e.target.value)} />
          </>
        }
